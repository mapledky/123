...
class _3DFront(Dataset):
    ...
    def read_entries(self, split, data_root, d_slice=None, shuffle=True):
        if split == 'train':
            file_s = [8000,7000,3500,5000]
            #file_s = [100,28,60,20]
            entries = self._build_data_list(os.path.join(data_root, 'rawdata/sp/high'), file_s[0])
            entries.extend(self._build_data_list(os.path.join(data_root, 'rawdata/sp/low'), file_s[1]))
            entries.extend(self._build_data_list(os.path.join(data_root, 'rawdata/bp/high'), file_s[2]))
            entries.extend(self._build_data_list(os.path.join(data_root, 'rawdata/bp/low'), file_s[3]))
            if d_slice:
                entries = entries[:d_slice]
        elif split == 'val':
            file_s = [500,500,500,500]
            #file_s = [100,28,60,20]
            entries = self._build_data_list(os.path.join(data_root, 'test/sp/high'), file_s[0])
            entries.extend(self._build_data_list(os.path.join(data_root, 'test/sp/low'), file_s[1]))
            entries.extend(self._build_data_list(os.path.join(data_root, 'test/bp/high'), file_s[2]))
            entries.extend(self._build_data_list(os.path.join(data_root, 'test/bp/low'), file_s[3]))
            if d_slice:
                entries = entries[:d_slice]
        else:
